...
Each implementation agent must run relevant local validation, push the final intended branch head, open/update the PR, record the exact head SHA, and observe `.github/workflows/ci.yml`.
...
If CI fails, keep the lane active, diagnose/fix the real defect on the task branch, push a new head SHA and repeat. Native DWG/CAD/runtime gates that cannot be proven by repository-safe CI remain explicitly `BLOCKED`/handed off; do not promote CI to native PASS.
...
<task result plus exact-head CI success, without overstating native evidence>
```
